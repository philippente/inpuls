...
class GPSTrainingGUI(object):
    ...
    def __init__(self, hyperparams):
        ...
        actions_arr = [
            ...
            Action('gcm', 'train gcm', self.request_train_gcm, axis_pos=1),
            Action('go',    'go',    self.request_go,    axis_pos=2),
            Action('test',  'test traj', self.request_test_traj, axis_pos=3),
        ]
    ...
    def request_train_gcm(self, event=None):
        print("gcm")
        self.request_mode('gcm')

    def request_go(self, event=None):
        print("go")
        self.request_mode('go')

    def request_test_traj(self, event=None):
        print("test")
    ...
    def process_mode(self):
        ...
        if self.request in ('stop', 'reset', 'fail'):
            self.wait_mode()
        elif self.request in ('go', 'gcm', 'test'):
            self.run_mode()
        self.request = None
