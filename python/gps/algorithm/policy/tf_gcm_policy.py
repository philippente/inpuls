...
class TfGCMPolicy(Policy):
    ...
    def act(self, x, obs, t, noise):
        ...
        gain_mean, jac_K = self.gcm._sess.run([self.gcm.ctr_stabilizer, self.gcm.jac_stabilizer], feed_dict={self.gcm.beta_kl: 1, self.gcm.train: 0, self.gcm.ctrgen_x: obs})
        offset_mean, jac_k = self.gcm._sess.run([self.gcm.ctr_action, self.gcm.jac_action], feed_dict={self.gcm.beta_kl: 1, self.gcm.train: 0, self.gcm.ctrgen_x: obs})

        K = np.reshape(gain_mean[0, :], [self.dU, self.dX])
        k = np.reshape(offset_mean[0, :], [self.dU])
        action_mean = K.dot(x) + k
    ...
    def pickle_policy(self, deg_obs, deg_action, checkpoint_path, goal_state=None, should_hash=False):
        ...
        saver.save(self.gcm._sess, checkpoint_path + '_tf_data')
