...
class AlgorithmBADMM(Algorithm):
    ...
    def iteration(self, sample_lists, _):
        ...
        for m in range(self.M):
            ...
            print("initial kl: ", self.cur[m].pol_info.init_kl)
            #org_traj_dist[m] = copy.deepcopy(self.cur[m].traj_distr)
        ...
        for inner_itr in range(self._hyperparams['inner_iterations']):
            ...
            if self.iteration_count > 0 or inner_itr > 0:
                # Update the policy. Creates samples based on a modified linear feedback policy to train the neural network policy.
                self._update_policy(inner_itr)
            for m in range(self.M):
                self._update_policy_fit(m)  # Update policy priors. This means: train a GMM as a policy prior and linearize policy
            if self.iteration_count > 0 or inner_itr > 0:
            ...
            if inner_itr == 0:
                org_traj_dist = copy.deepcopy(self.new_traj_distr)

        self._advance_iteration_variables()
        # self.traj_distr = org_traj_dist
        self.new_traj_distr = copy.deepcopy(org_traj_dist)
    ...
    def _estimate_cost(self, traj_distr, traj_info, pol_info, m):
        ...
        print("predicted_kl: ", predicted_kl)
